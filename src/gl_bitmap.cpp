...
void GL_Bitmap::draw()
{
	...
	glBegin(GL_POLYGON);
	glTexCoord2f(0.0f, 0.0f); glVertex2d(m_area.left, m_area.bottom);//左下
	glTexCoord2f(0.0f, 1.0f); glVertex2d(m_area.left, m_area.top);//左上
	glTexCoord2f(1.0f, 1.0f); glVertex2d(m_area.right, m_area.top);//右上
	glTexCoord2f(1.0f, 0.0f); glVertex2d(m_area.right, m_area.bottom);//右下
	glEnd();
