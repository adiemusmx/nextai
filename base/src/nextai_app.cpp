...
namespace NextAI
{
	...
	void AppService::init(AppServiceParam& param)
	{
		...
		NEXTAI_TRACE_LOG(L"App workspace[{}]", path);
		// Render
	...
	{
		NEXTAI_TRACE_LOG(L"listener[{}]", (void*)listener);
		...
		{
			NEXTAI_WARN_LOG(L"Listener[{}] duplicate.", (void*)listener);
		}
	}
	...
	void AppService::removeEventListener(AppEventListener* listener)
	{
		...
		{
			NEXTAI_WARN_LOG(L"Listener[{}] not found.", (void*)listener);
		}
	}
	...
	{
		NEXTAI_TRACE_LOG(L"GLUT key[{}] pos[{},{}]", key, x, y);
	...
	void AppService::mouseFunc(int button, int state, int x, int y)
	{
		...
		{
			NEXTAI_TRACE_LOG(L"[Gesture][TouchType_BEGAN] pos[{},{}]", x, y);
			VECTOR_NOTIFY(instance()->m_listeners, touch, TouchType_BEGAN, 1, touchId, touchPos);
		}
		else if (state == GLUT_UP && GLUT_LEFT_BUTTON == button)
		{
			NEXTAI_TRACE_LOG(L"[Gesture][TouchType_ENDED] pos[{},{}]", x, y);
	...
	void AppService::motionFunc(int x, int y)
	{
		...
		touchPos[0].x = x;
		touchPos[0].y = y;
		NEXTAI_TRACE_LOG(L"[Gesture][TouchType_MOVED] pos[{},{}]", x, y);
